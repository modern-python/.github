...
- [x] Every rewritten profile blurb equals the repo's GitHub description
      character-for-character (`gh repo view` comparison).
- [x] All 12 canonical strings are ≤120 chars, carry no trailing period, and
      contain no `|` (would break the table).
- [x] `just check-planning` — clean.
- [x] Both template `pyproject.toml` descriptions match their GitHub description.
