...
- **Medium (realized, fixed): CREAM must only knock out of GOLD, never sit
  ...
  (`lockup-dark.svg` is the dark half of every README `<picture>` banner), so
  ...
  `test_new_marks_have_no_cream_on_transparent` added as the regression guard.
  The durable lesson: composite every new mark onto a dark background before
  it ships, not just a light one.
